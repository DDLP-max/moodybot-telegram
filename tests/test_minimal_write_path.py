...
from response_finalization import CORE_WRITE_DIRECTIVE, finalize_response, plan_closer_instruction
...
def test_core_write_directive_is_insight_first():
    ...
    assert "interesting true thing" in text.lower() or "underlying rule" in CORE_WRITE_DIRECTIVE.lower()
    assert "signature line" in text.lower()
    assert "do not add a signature line" in text.lower()
